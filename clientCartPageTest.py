...
class CartPageTest(unittest.TestCase):
    def setUp(self):
        ...
        email.submit()
        time.sleep(2)
        self.driver.find_element(by=By.XPATH, value='//*[@id="root"]/footer/div/a[2]').click()
        time.sleep(2)
        warnings.filterwarnings(action="ignore", message="unclosed", category=ResourceWarning)

    def test_current_url(self):
        current_url = self.driver.current_url
        self.assertIn(current_url, "http://localhost:3000/cart")

    def test_get_page_heading(self):
        elem = self.driver.find_element(by=By.XPATH, value='//*[@id="root"]/div[2]/div/div/h2')
        time.sleep(2)
        self.assertIn(elem.text, "My Cart")
    ...
    def test_get_items_if_no_item(self):
        elem = self.driver.find_element(by=By.XPATH, value='//*[@id="root"]/div[2]/p')
        time.sleep(2)
        self.assertIn(elem.text, "No items in your cart")
    ...
    def test_get_filled_cart(self):
        ...
        time.sleep(5)
        self.driver.execute_script("arguments[0].scrollIntoView(true);", elem)
        self.assertIn(elem.text, "CANCEL THIS TRIP") 
